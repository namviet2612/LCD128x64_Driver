...
void GLCD_SetSide(unsigned char Side)
...
void GLCD_SetYADDRESS(uint8_t Col)
{ //set Y address (or column) of GLCD
    wait_GLCD();
    GLCD_OUT_Set();
    cbi(PORT_1, GLCD_RS); //pull both RS, RW down, (AVR->GLCD)
    cbi(PORT_0, GLCD_RW);
...
void GLCD_SetXADDRESS(uint8_t Line)
{ //set X address (or line) of GLCD
    wait_GLCD();
    GLCD_OUT_Set();
    cbi(PORT_1, GLCD_RS); //pull both RS, RW down,(AVR->GLCD)
    cbi(PORT_0, GLCD_RW);
...
void GLCD_StartLine(uint8_t Offset)
{ //Set start line khi cuon GLCD
    wait_GLCD();
    GLCD_OUT_Set();
    cbi(PORT_1, GLCD_RS); //pull both RS, RW down, (AVR->GLCD)
    cbi(PORT_0, GLCD_RW);
...
void GLCD_GotoXY(uint8_t Line, uint8_t Col)
{ //Line:0-7, Col: 0-127
    uint8_t Side;
...
void GLCD_PutChar_setup(uint8_t Line, uint8_t Col, uint16_t chr)
{
    ...
    if ((Col > (64 - font_setup[chr - 28])) && (Col < 64))
    {                           //there is a "jump" from left->right
        GLCD_GotoXY(Line, Col); //left first
        for (i = 0; i < 64 - Col; i++)
...
void GLCD_PutChar_prog(uint8_t Line, uint8_t Col, uint16_t chr)
{
    ...
    if ((Col > (64 - font_ten_ct[chr - 28])) && (Col < 64))
    {                           //there is a "jump" from left->right
        GLCD_GotoXY(Line, Col); //left first
        for (i = 0; i < 64 - Col; i++)
